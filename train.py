...
from AUC import caculate_AUC
from AUPR import AUPR_caculate
###############################################################
# Training settings
...
def train(epoch, link_dti_id_train, edge_index, edge_weight, train_dti_inter_mat):
    ...
    output = model(protein_tensor, drug_tensor, edge_index, protein_index, drug_index, edge_weight)
    Loss = nn.BCELoss()
    # if DTA task
    # Loss = nn.MSELoss()
...
for train_times in range(fold_num):
    ...
    for epoch in range(args.epochs):
        ...
        if test_score > best_test:
            ...
            caculate_AUC(targets.cpu().detach().numpy(),predicts.cpu().cpu().detach().numpy())
            AUPR_caculate(targets.cpu().detach().numpy(),predicts.cpu().cpu().detach().numpy())
        print('Epoch: {:04d}'.format(epoch + 1), 'Train_times:', train_times)
